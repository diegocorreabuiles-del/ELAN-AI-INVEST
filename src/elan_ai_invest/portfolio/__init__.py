...
def build_portfolio(
    ...
    if "volatility_pct" in candidates.columns:
        ...
        if "volatility_pct" not in risk_data.columns:
            if "annual_volatility_pct" in risk_data.columns:
                risk_data["volatility_pct"] = risk_data[
        ...
        volatility_map = risk_data.set_index(
            "symbol"
        )["volatility_pct"]

        volatility = candidates["symbol"].map(volatility_map)

    candidates["volatility_pct"] = pd.to_numeric(
        volatility,
        errors="coerce",
    ).fillna(20.0).clip(lower=1.0)
    ...
    if raw_total <= 0:
        candidates["weight_pct"] = (
            available_weight / len(candidates)
        )
    else:
        candidates["weight_pct"] = (
            candidates["raw_weight"]
    ...
    candidates["weight_pct"] = candidates[
        "weight_pct"
    ].clip(upper=max_position_pct)
...
def portfolio_equity_curve(
    ...
    symbols = [
        symbol
        for symbol in allocations["symbol"]
