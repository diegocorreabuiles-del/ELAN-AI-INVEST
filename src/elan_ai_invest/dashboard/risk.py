...
def render_risk_tab(risk_report, ranking, capital, settings):
    ...
    risk_table["suggested_position_pct"] = risk_table["volatility_pct"].apply(lambda value: suggested_position_size_pct(value, risk_budget_pct=settings.risk.risk_budget_per_position_pct, max_position_pct=settings.risk.max_position_pct))
    risk_table["suggested_amount_eur"] = risk_table["suggested_position_pct"] / 100 * capital
    st.dataframe(risk_table, use_container_width=True, hide_index=True)
    st.plotly_chart(px.imshow(risk_report.correlation, text_auto=".2f", zmin=-1, zmax=1, aspect="auto", title="Correlaciones"), use_container_width=True)
