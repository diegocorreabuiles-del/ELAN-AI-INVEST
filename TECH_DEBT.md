...
> **Revisión tras la migración de PC (22 de julio de 2026).** TD-015, TD-016, TD-017, TD-018, TD-023, TD-025, TD-028, TD-032 y la gobernanza TD-019 están verificadas: healthcheck SQLite real, lock, empaquetador, AppTest, errores seguros, gate Git, datos de riesgo alineados, tema nativo y matriz Linux Python 3.11–3.14. La rama de hardening pasa 121 pruebas y alcanza 81,0 % de cobertura local; Yahoo queda aislado en AppTest y SQLite usa rutas temporales. Las afirmaciones posteriores se conservan como registro histórico y solo coinciden con el estado actual cuando esta nota las confirma.

> Estado v1.2.2: TD-001, TD-002, TD-003, TD-004, TD-005, TD-006, TD-008, TD-009, TD-011, TD-013, TD-014, TD-016, TD-017, TD-018 y TD-022 quedan resueltos. TD-007 queda mitigado con `CoreEngine` canónico y legacy congelado; TD-019 queda mitigado con política y gate `trabajo -> develop -> main`, a la espera de integración autorizada. TD-012, TD-024, TD-028, TD-029 y TD-034 quedan parcial o totalmente tratados según `AUDIT_REPORT.md`; TD-025 queda resuelto con tema nativo y sin CSS inyectado. El resto sigue pendiente.
...
| TD-016 | P1 | Reproducibilidad | Resuelto: `requirements.lock` fija el cierre transitivo y `requirements.txt`, CI e instaladores lo consumen | `check_lock.py` valida 76 pins activos; la matriz Docker 3.11–3.14 pasa con `pip check` | Mantener el lock y ejecutar `scripts/run_ci_matrix.ps1` antes de cada release |
...
| TD-018 | P1 | Pruebas | Resuelto tras la recuperación: el gate local de hardening supera 121 pruebas, cubre líneas y ramas al 81,0 % y mantiene el umbral CI de 75 % | AppTest ejecuta `app.py`, las once vistas y acciones paper simuladas con datos deterministas; Yahoo queda aislado y SQLite usa rutas temporales | Aumentar el umbral gradualmente y cubrir rutas de error/legacy solo según riesgo demostrado |
| TD-019 | P1 | Ramas | Resuelto en gobernanza: `check_git_flow.py`, CI y protecciones aplican `trabajo -> develop -> main`; PR #1 y PR #2 integrados en `develop` con CI posterior verde | `main` sigue en 0.1 y no se ha autorizado su integración; el bypass administrativo queda solo para recuperación | Preparar PR independiente `develop -> main` únicamente tras cerrar el mantenimiento CI y con autorización explícita |
