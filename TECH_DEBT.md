...
> **Revisión tras la migración de PC (21 de julio de 2026).** TD-016, TD-017, TD-018 y la parte local de TD-019 vuelven a estar verificadas: el lock, el empaquetador, AppTest y el gate Git han sido reconstruidos. Pasan 115 pruebas con 80,6 % de cobertura; Yahoo y las bases reales quedan aislados en AppTest. Las afirmaciones posteriores se conservan como registro del ordenador anterior y solo coinciden con el estado actual cuando esta nota las confirma.
...
| TD-018 | P1 | Pruebas | Resuelto tras la recuperación: 115 pruebas, cobertura de líneas y ramas del producto de 80,6 % y umbral CI de 75 % | AppTest ejecuta `app.py`, las once vistas y acciones paper simuladas con datos deterministas; Yahoo y las bases reales quedan aislados | Aumentar el umbral gradualmente y cubrir rutas de error/legacy solo según riesgo demostrado |
