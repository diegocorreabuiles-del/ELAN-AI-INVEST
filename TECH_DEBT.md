...
> Estado v1.2.2: TD-001–TD-006, TD-008–TD-011, TD-013–TD-020 y TD-022–TD-034 quedan resueltos o mitigados salvo TD-012 y TD-021. TD-007 queda mitigado con `CoreEngine` canónico y legacy congelado; TD-026 abre un baseline de mypy y queda pendiente ampliar su alcance. TD-019 mantiene pendiente únicamente la promoción autorizada `develop -> main`.
...
| TD-016 | P1 | Reproducibilidad | Resuelto: `requirements.lock` fija el cierre transitivo y `requirements.txt`, CI e instaladores lo consumen | `check_lock.py` valida 78 pins activos y `pip check` pasa | Mantener el lock y ejecutar `scripts/run_ci_matrix.ps1` antes de cada release |
...
| TD-019 | P1 | Ramas | Resuelto en gobernanza: `check_git_flow.py`, CI y protecciones aplican `trabajo -> develop -> main`; PR #1–#4 integrados en `develop` con CI posterior verde | `main` sigue sin promoción autorizada; el bypass administrativo queda solo para recuperación | Abrir PR independiente `develop -> main` tras cerrar el release candidate y esperar autorización explícita para fusionar |
...
| TD-026 | P2 | Tipado | Mitigado: mypy estricto cubre 12 módulos críticos de core, cartera, riesgo, paper, sistema y UI segura en local y CI | Los contratos críticos ya bloquean la integración; el resto del árbol aún no está cubierto | Ampliar el alcance gradualmente sin ocultar errores con exclusiones globales |
| TD-027 | P2 | Dependencias | Resuelto: `python-dotenv` no tenía consumidor directo ni transitivo y se retiró del proyecto y del lock | Menor superficie; `check_lock.py` y `pip check` pasan con 78 distribuciones | Mantener revisión de consumidores antes de retirar otras dependencias |
