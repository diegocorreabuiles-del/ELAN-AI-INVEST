...
| TD-019 | P1 | Ramas | Resuelto en gobernanza: `check_git_flow.py`, CI y protecciones aplican `trabajo -> develop -> main`; PR #1 publicado, verde y listo para revisión | `main` sigue en 0.1 y no se ha autorizado ningún merge; el bypass administrativo queda solo para recuperación | Integrar PR #1 en `develop` con autorización; después PR independiente `develop -> main` con gates verdes |
