...
| TD-019 | P1 | Ramas | Resuelto en gobernanza: `check_git_flow.py`, CI y protecciones aplican `trabajo -> develop -> main`; PR #1 integrado en `develop` con CI posterior verde | `main` sigue en 0.1 y no se ha autorizado su integración; el bypass administrativo queda solo para recuperación | Preparar PR independiente `develop -> main` únicamente tras cerrar el mantenimiento CI y con autorización explícita |
