...
> **Revisión tras la migración de PC (21 de julio de 2026).** TD-016, TD-017, TD-018, TD-028 y la parte local de TD-019 vuelven a estar verificadas: lock, empaquetador, AppTest, gate Git y matriz Linux Python 3.11–3.14. Pasan 116 pruebas con 80,6–80,7 % de cobertura; Yahoo y las bases reales quedan aislados en AppTest. Las afirmaciones posteriores se conservan como registro del ordenador anterior y solo coinciden con el estado actual cuando esta nota las confirma.
...
| TD-018 | P1 | Pruebas | Resuelto tras la recuperación: 116 pruebas, cobertura de líneas y ramas del producto de 80,6–80,7 % y umbral CI de 75 % | AppTest ejecuta `app.py`, las once vistas y acciones paper simuladas con datos deterministas; Yahoo y las bases reales quedan aislados | Aumentar el umbral gradualmente y cubrir rutas de error/legacy solo según riesgo demostrado |
| TD-019 | P1 | Ramas | Mitigado: `check_git_flow.py`, CI y Dependabot aplican `trabajo -> develop -> main`; la candidata está consolidada y desciende del `origin/develop` actualizado | `main` sigue en 0.1 y la rama de recuperación no tiene upstream porque no se autorizan push/merge; protecciones remotas no verificadas | Publicar la rama y crear PR a `develop`; después PR `develop -> main`, solo con autorización y gates remotos verdes |
...
| TD-028 | P2 | Python | Resuelto localmente: CI y `run_ci_matrix.ps1` cubren Python 3.11–3.14 | Las cuatro versiones pasan lock, lint, formato, pruebas, cobertura y empaquetado en Linux; GitHub Actions remoto aún no ejecutado | Mantener ambas matrices alineadas y verificar el PR remoto antes de release |
