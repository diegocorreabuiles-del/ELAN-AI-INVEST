...
- Ampliación global FX local: 31/31 pruebas dirigidas de catálogo, routing, sincronizador y contratos, más AppTest integral de las 13 vistas; Ruff, Black, mypy dirigido y `git diff --check` verdes. Yahoo real validó AFN, IDR, NGN, QAR y XOF y el cruce sintético NGN/XOF vía USD; app local HTTP 200 en `8501`.
- Catálogo principal FX unificado local: pruebas dirigidas de instrumentos y clasificación, AppTests de búsqueda global, filtros Divisa/país/mercado y recorrido de las 13 vistas; Ruff, Black, `py_compile` y `git diff --check` verdes. App local HTTP 200 en `8501`.
- Fase 10, integración FX en motor principal: 322/322 pruebas y cobertura global de ramas 82,72 %; mypy, Ruff, Black del código activo, lock, `pip check`, `git diff --check`, AppTest `NGN/XOF` y HTTP 200 en `8501` verdes. El ajuste posterior para conservar posiciones con universo solo FX pasó 2/2 AppTests dirigidos. El ZIP reproducible queda pendiente porque el constructor exige working tree confirmado; no se creó commit, push, fusión ni despliegue.
- Fase 11, trazabilidad FX: 299/299 pruebas no-AppTest y 24/25 AppTests verdes por lotes; cobertura combinada de ramas 75,5 % (mínimo 75 %), 17/17 pruebas dirigidas, Ruff, Black y `git diff --check` verdes. El AppTest monolítico de las 13 vistas volvió a bloquearse sin excepción y se conserva como gate pendiente; no se creó commit, push, fusión ni despliegue.
...
22. **Motor FX:** `config/currencies.csv` es el catálogo maestro; `scripts/sync_currency_catalog.py` contrasta ISO 4217 de SIX con Yahoo y solo habilita símbolos con histórico mínimo. Los pares virtuales `FX_BASE_QUOTE` se buscan bajo demanda y `FxAwareMarketDataProvider` los resuelve directo, inverso o por rutas cortas USD/EUR, normaliza fechas y los combina con símbolos Yahoo para análisis, scoring/ranking y riesgo. Yahoo es el único proveedor y falla por instrumento; disponibilidad no implica convertibilidad ni negociabilidad. La caché es CSV inerte. No hay tablas FX/Supabase y FX permanece excluido de Portfolio, paper trading, brokers y dinero real.
23. **Comparador de Mercado:** mantiene una selección de entre dos y ocho instrumentos focales. La matriz cubre todos los seleccionados y la correlación móvil compara una referencia elegible contra cada uno de los demás; no se reduce a un par focal A/B.
24. **Trazabilidad FX en Mercado:** `MarketDataAssetQuality` admite metadata opcional de proveedor, ruta, cobertura de ruta y recepción. `FxAwareMarketDataProvider` la completa solo para pares virtuales y Mercado la presenta sin convertirla en input de scoring ni alterar proveedores ordinarios.
